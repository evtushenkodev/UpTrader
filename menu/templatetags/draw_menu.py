...
from menu.models import FoodMenuItem

register = template.Library()


@register.inclusion_tag('../templates/foodmenu/foodmenu.html', takes_context=True)
def draw_menu(context: Context, menu: str) -> Dict[str, Any]:
    ...
    :return: A dictionary that will be used to render the foodmenu/foodmenu.html template.
    """

    try:
        # Get all menu items for the given menu
        items = FoodMenuItem.objects.filter(menu__name=menu)
        items_values = items.values()

        # Get the root menu items (without parent_item)
        root_item = [item for item in items_values.filter(parent_item=None)]
        ...
        # In case of an error, return a list of menu items without parent_item items
        result_dict = {
            'items': [
                item for item in FoodMenuItem.objects.filter(menu__name=menu, parent_item=None).values()
            ]
        }
...
def get_child_items(items_values, current_item_id, selected_item_id_list):
    ...
    """
    item_list = [item for item in items_values.filter(parent_item_id=current_item_id)]
...
def get_selected_item_id_list(parent_item: FoodMenuItem, primary_item: List[FoodMenuItem], selected_item_id: int) -> List[int]:
    """
    Returns a list of IDs for the selected menu items, starting from the parent_item item to the current one.

    :param parent_item: The parent_item menu item.
